...
function collect(path) {
  ...
  for (const entry of entries) {
    ...
    if (
      child === resolve("docs/evidence") ||
      child === resolve("docs/research-assessment.md")
    )
      continue;
...
for (const file of files) {
  ...
  for (const match of source.matchAll(/\[[^\]]+\]\(([^)]+)\)/g)) {
    ...
    const target = rawPath
      ...
      : file;
    if (extname(target).toLowerCase() === ".md" && !sourceByFile.has(target)) {
      errors.push(`${shown}: link outside public documentation ${destination}`);
      continue;
    }
    if (!existsSync(target)) {
      errors.push(`${shown}: broken link ${destination}`);
      continue;
    }
    if (rawFragment && extname(target).toLowerCase() === ".md") {
      const targetSource =
        sourceByFile.get(target) ?? readFileSync(target, "utf8");
      const fragment = decodeURIComponent(rawFragment).toLowerCase();
      if (!headingSlugs(targetSource).has(fragment))
        errors.push(`${shown}: broken heading link ${destination}`);
    }
  }
  let fenceOpen = false;
  for (const match of source.matchAll(/^```([^\r\n]*)$/gm)) {
    ...
    if (!fenceOpen && !info)
      errors.push(`${shown}: code fence is missing a language identifier`);
    fenceOpen = !fenceOpen;
  }
  if (!shown.startsWith("development-standards")) {
    for (const pattern of [
      /\bas needed\b/giu,
      /\bwhen appropriate\b/giu,
      /\bsimply\b/giu,
      /\bobviously\b/giu,
      /\bjunior (?:administrator|admin|developer|user)\b/giu
    ]) {
      const match = pattern.exec(source);
      if (match) {
        const line = source.slice(0, match.index).split(/\r?\n/u).length;
        errors.push(
          `${shown}:${line}: replace vague or reader-label wording “${match[0]}” with a prerequisite or decision rule`
        );
      }
    }
  }
