...
export function encodeUtf8Base64(value, Reader = FileReader) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      if (separatorIndex < 0) {
        reject(new Error(labels.CsvReadError));
        return;
      }
      resolve(dataUrl.slice(separatorIndex + 1));
    };
    reader.onerror = () => reject(new Error(labels.CsvReadError));
    reader.readAsDataURL(new Blob([value], { type: "text/csv;charset=utf-8" }));
  });
}
