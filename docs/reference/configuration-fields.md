...
`ContextFilterCriteria__c` holds one condition per line, `FieldApiName:Operator:Value`, where `Operator` is `=`, `!=`, or `IN` (comma-separated values for `IN`); every line combines with AND, up to 10 lines. **Example:** to allow only open Opportunities, use two lines:
...
The value selected by Developer Name in Lightning App Builder when placing the **Bulk Record
Upload** component. See
[Configure an upload process](../admin/configure-upload-process.md#put-the-component-on-a-page) for
