...
| `VALIDATION_ERROR`                      | A configured field rule or value check rejected the row before Salesforce DML.                                                                                                          |
| `MATCH_ERROR`                           | The configured match key was missing, ambiguous, unavailable, or did not resolve to a record.                                                                                           |
| `CONFIGURATION_CHANGED`                 | The upload's saved configuration no longer matches the current process configuration, so processing stopped safely.                                                                     |
| `EXTENSION_ERROR`                       | A registered package extension did not complete successfully.                                                                                                                           |
| `ABANDONED_UPLOAD`                      | Recovery marked work failed because it remained active beyond the supported processing window.                                                                                          |
