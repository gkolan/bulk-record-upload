...
## Lightning page contract

The single exposed `bulkRecordUpload` component supports Lightning Record Pages and Lightning App Pages. Record pages support one selected process, a configured process group, or all active processes for the current object. App pages support the same process modes plus one configured record, multiple independent records, or a user-selected record. There is no Home Page, Lightning tab, or Experience Cloud target in the Day-1 contract.

See [Configure Lightning pages](../admin/configure-lightning-pages.md) for the App Builder properties and verification steps.
