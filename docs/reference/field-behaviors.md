...
**Use case:** Priya runs sales ops. Every Friday, each rep's call notes get exported from the team's tracking sheet and uploaded as a CSV, one row per account, one `call_note` column. She wants Account `Description` to read like a call log — this week's note added under last week's, oldest at the top — not replaced every Friday.
...
The framework only joins whatever text is in the CSV cell — it doesn't stamp a date or a name on it. Priya's export already formats each note as `date — rep initials: text`, the same way her team would type it by hand. That's what makes the log readable later.
...
| Week 2 (a different rep)                      | `2026-08-14 — JT: Left voicemail re: upcoming QBR, no callback yet.`    | Week 1's line, then `2026-08-14 — JT: Left voicemail re: upcoming QBR, no callback yet.` on the next line |
...
**Use case:** Priya's manager only ever reads the most recent note and doesn't want to scroll to the bottom of `Description` to find it. Same weekly file, same column — just the newest note on top instead of the oldest.
...
Works the same way on Percent fields — a weekly Opportunity `Probability` adjustment file could `ADD` or `SUBTRACT` a few points instead of overwriting a number a rep already fine-tuned by hand.
...
**Use case:** A contact-import file sometimes has a garbled `Email` column from a bad export, and a `NumberOfEmployees` column with a placeholder value that's obviously wrong. Both should fail just those rows, not corrupt the data or block the rest of the file.
