...
Run `npm run benchmark:large-schema` to measure compact projections using 800 synthetic field descriptors and the supported 100-field maximum. The script prints its environment, budgets, and measurements. This checks the local projection model, not end-to-end Salesforce throughput. See the [benchmark source](../../scripts/run-large-schema-benchmark.mjs) and [testing guide](testing.md).
