...
- Extensions (both the row-extension seam and custom merge strategies) are the only subscriber-written Apex the pipeline ever calls. The operation (Insert/Update/Upsert/Delete) and the built-in save path are fixed, reviewed code — they aren't extensible, by design (see [ADR-0007](../../specs/decisions/ADR-0007-configuration-over-code-extension.md)).
